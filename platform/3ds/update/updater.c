...
static void UpdateLog(const char *format, ...) {
  ...
  FILE *f = fopen("sdmc:/3ds/MK64/update.log", "a");
  if (f) { fprintf(f, "%s\n", line); fclose(f); }
}

#define UPDATE_DIR "sdmc:/3ds/MK64/update"
...
bool Updater_ShouldClose(void) { if (Updater_Busy()) return false; UpdateStatus s; Updater_GetStatus(&s); return s.state == UPDATE_DONE; }
void Updater_Cancel(void) {
  if (!initialized) return;
  LightLock_Lock(&lock);
  if (status.state != UPDATE_INSTALLING)
    __atomic_store_n(&cancel, true, __ATOMIC_RELEASE);
  LightLock_Unlock(&lock);
}
...
static bool fetch(const char *url, Transfer *t) {
  ...
  curl_easy_setopt(c, CURLOPT_USERAGENT, "Mario-Kart-64-3DS/" MK64_3DS_VERSION);
...
static bool install_cia(void) {
  ...
  rc = FSUSER_OpenFileDirectly(&input, ARCHIVE_SDMC, fsMakePath(PATH_EMPTY, ""),
    fsMakePath(PATH_ASCII, "/3ds/MK64/update/download.part"), FS_OPEN_READ, 0);
  if (R_FAILED(rc)) goto done;
  rc = AM_GetCiaFileInfo(MEDIATYPE_SD, &info, input);
  if (R_FAILED(rc) || info.titleID != 0x0004000005a27000ull) goto done;
...
static void run_job(void *arg) {
  ...
  if (!download_job) {
    ...
    bool newer = Update_IsNewer(release.version, MK64_3DS_VERSION);
...
static void start(bool downloading) {
  ...
  // The Core 0 renderer can remain runnable between frames. Unlike the
